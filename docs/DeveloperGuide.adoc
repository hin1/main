...
(userId), the unique identifier for that particular loan (reservationId) to identify multiple unique Resources
borrowed, and lastly two Dates to identify the duration for which the Resource is being borrowed.
...
then parses it (using parseInput method) to identify the type of Command it is. After identifying the type of Command,
Parser then evaluates the relevant parameters required and constructs the corresponding Command, using the relevant Parser method. Following that, the
...
when to terminate. The Boolean attribute willExit is inherited in every subclass of Command, and helps the main program
